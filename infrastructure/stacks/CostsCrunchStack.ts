...
export class CostscrunchStack extends Stack {
  constructor(scope: Construct, id: string, props: CostscrunchStackProps) {
    ...
    const prefix = `Costscrunch-${env}`;

    // ── KMS Key ─────────────────────────────────────────────────────────────
    const kmsKey = new kms.Key(this, "KmsKey", {
      alias: `${prefix}/main`,
      enableKeyRotation: true,
      description: "Costscrunch primary encryption key",
    ...
    // VPC Interface Endpoints (keep traffic off public internet)
    for (const service of [
      ...
      new ec2.InterfaceVpcEndpoint(this, `Ep${service.name.replace(/\./g, "")}`, {
        vpc,
        service,
    ...
    const receiptsBucket = new s3.Bucket(this, "ReceiptsBucket", {
      ...
      cors: [{
        ...
        allowedOrigins: isProd ? ["https://app.Costscrunch.io"] : ["*"],
        allowedHeaders: ["*"],
        maxAge: 3600,
      }],
      lifecycleRules: [
        { transitions: [{ storageClass: s3.StorageClass.INTELLIGENT_TIERING, transitionAfter: Duration.days(30) }] },
        { expiration: Duration.days(2555), noncurrentVersionExpiration: Duration.days(90) },
      ],
    });
    ...
    new cognito.CfnUserPoolGroup(this, "ProGroup", { userPoolId: userPool.userPoolId, groupName: "pro", precedence: 2 });
    ...
    const expensesLambda = new NodejsFunction(this, "ExpensesLambda", {
      ...
      entry: "backend/lambdas/expenses/index.ts",
    ...
    const groupsLambda = new NodejsFunction(this, "GroupsLambda", {
      ...sharedLambdaProps as any,
      entry: "backend/lambdas/groups/index.ts",
    ...
    const receiptsLambda = new NodejsFunction(this, "ReceiptsLambda", {
      ...sharedLambdaProps as any,
      entry: "backend/lambdas/receipts/index.ts",
    ...
    const analyticsLambda = new NodejsFunction(this, "AnalyticsLambda", {
      ...sharedLambdaProps as any,
      entry: "backend/lambdas/analytics/index.ts",
    ...
    const notificationsLambda = new NodejsFunction(this, "NotificationsLambda", {
      ...sharedLambdaProps as any,
      entry: "backend/lambdas/notifications/index.ts",
    ...
    const api = new apigwv2.HttpApi(this, "Api", {
      apiName: `${prefix}-api`,
      corsPreflight: {
        allowOrigins: isProd ? ["https://app.Costscrunch.io"] : ["*"],
        allowMethods: [
          apigwv2.CorsHttpMethod.GET, apigwv2.CorsHttpMethod.POST,
          apigwv2.CorsHttpMethod.PATCH, apigwv2.CorsHttpMethod.DELETE,
        ],
